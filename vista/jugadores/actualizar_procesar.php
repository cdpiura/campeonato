...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Verificar si el código de deportista ya está registrado
    ...
    if (pg_num_rows($result_verificar) > 0) {
        echo "<script>
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = 'insertar_movimiento.php?iddeportista=$iddeportista&fecha=$fecha&rojas=$rojas&amarillas=$amarillas&goles=$goles';
                } else {
                    window.history.back();
                }
            });
    ...
    // Insertar un nuevo movimiento si no está registrado
